...
def rename_texture_files():
    ...
    # Function to ensure unique name within a directory
    def get_unique_name(directory, base_name):
        unique_name = base_name
        counter = 1
        while os.path.exists(os.path.join(directory, unique_name)):
            unique_name = f"{base_name.split('.')[0]}.{counter}.png"
            counter += 1
        return unique_name
        ...
        if material.use_nodes:
            ...
            for node in nodes:
                ...
                if node.type == 'TEX_IMAGE':
                    ...
                    # Define the new base name based on the material name
                    base_name = material.name.replace(' ', '_').replace(':', '').replace('"', '') + '.png'
                    
                    # Create a unique file name for unpacked images
                    textures_directory = bpy.path.abspath("//textures")
                    new_file_name = get_unique_name(textures_directory, base_name)
                    
                    if image.packed_file:
                        # Rename the packed image data-block within Blender
                        image.name = new_file_name
                        
                        # Modify the packed file path
                        new_file_path = os.path.join("//textures", new_file_name)
                        image.filepath = new_file_path
                        print(f'Renamed packed image to {new_file_name} and set filepath to {new_file_path}')
                    else:
                        ...
                        new_file_name = get_unique_name(os.path.dirname(abs_image_path), material.name.replace(' ', '_').replace(':', '').replace('"', '') + '.png')
